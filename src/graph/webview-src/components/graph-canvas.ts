...
export class GraphCanvas extends LitElement {
  ...
  firstUpdated() {
    ...
    this.graphInstance = ForceGraph()(container)
      ...
      .d3Force(
        'citation-repel',
        forceManyBody<any>().strength((node: any) => {
          const info = this.rs.nodeInfo[node.id];
          return info?.type === 'citation' ? -120 : 0;
        })
      )
  ...
  private _drawNoteBoundary(ctx: CanvasRenderingContext2D): void {
    if (!this.graphInstance) return;
    const nodes: Array<{ x: number; y: number }> = [];
    (this.graphInstance.graphData().nodes as any[]).forEach((node: any) => {
      const info = this.rs.nodeInfo[node.id];
      if (info?.type === 'note' && node.x != null && node.y != null) {
    ...
    // Convert graph-space to screen-space using the canvas transform
    ...
    const fontSize = this.rs.style?.fontSize ?? 12;
    const fontFamily = this.rs.style?.fontFamily ?? 'sans-serif';
    ctx.font = `${Math.max(10, fontSize)}px ${fontFamily}`;
    ctx.fillText('Your notes', tl.x + r + 4, tl.y - 6);
    ctx.restore();
  }
