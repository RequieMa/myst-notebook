  ...
  const cachedParser: ResourceParser = {
    parse: (uri: URI, markdown: string): Resource => {
      ...
      if (cache!.has(uri)) {
        ...
        if (cached && actualChecksum === cached.checksum) {
...
const sectionsPlugin: ParserPlugin = {
  ...
  onDidVisitTree: (tree, note) => {
    const end = Position.create(
      astPointToFoamPosition(tree.position?.end ?? { line: 0, column: 0, offset: 0 }).line + 1,
      0
    );
  ...
  onDidFindProperties: (props, note, node) => {
    if (isSome(props.alias)) {
      const aliases = Array.isArray(props.alias)
        ? props.alias
        : props.alias.split(',').map((m: string) => m.trim());
...
const wikilinkPlugin: ParserPlugin = {
  ...
  visit: (node, note, noteSource) => {
    if (node.type === 'wikiLink') {
      const isEmbed =
        noteSource.charAt((node.position!.start.offset ?? 0) - 1) === '!';
